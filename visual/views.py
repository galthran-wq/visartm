...
def visual_global(request):
    ...
    if model is None:
        return general_views.message(
            request,
            "You have to create model first.<br>" +
            "<a href='/models/create?dataset=" +
            dataset.text_id +
            "'>Create model</a><br>" +
            "<a href='/dataset?dataset=" +
            dataset.text_id +
            "'>Return to dataset</a><br>")

    if 'try' in request.GET and request.GET['try'] == 'again':
        GlobalVisualization.objects.filter(
            model=model, name=visual_name).delete()
        return redirect("/visual/global?type=" +
                        visual_name + "&model=" + str(model.id))

    try:
        visualization = GlobalVisualization.objects.filter(
            model=model, name=visual_name)[0]
        ...
        if settings.THREADING:
            t = Thread(
                target=GlobalVisualization.render_untrusted, args=(
    ...
    if visualization.status == 0:
        ...
        return general_views.message(
            request,
            "Error during rendering.<br>" +
            visualization.error_message.replace(
                '\n',
                "<br>") +
            "<br><br><a href='/visual/global?type=" +
            visual_name +
            "&model=" +
            str(
                model.id) +
            "&try=again'>Try again</a>")

    data_file_name = os.path.join(
        model.get_visual_folder(),
        visual_name + ".txt")
    ...
    return render(
        request,
        "visual/" +
        visual_name.split('_')[0] +
        ".html",
        context)


def example(request, vis_name):
    models = ArtmModel.objects.filter(
        dataset__is_public=True,
        dataset__time_provided=True,
        layers_count__gte=2)
    if len(models) > 0:
        return redirect(
            "/visual/global?type=%s&model=%d" %
